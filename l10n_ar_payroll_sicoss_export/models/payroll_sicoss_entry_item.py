...
class PayrollSicossEntryItem(models.Model):
    ...
    sicoss_entry_id = fields.Many2one(
        'payroll.sicoss_entry', string='Liquidacion AFIP-SICOSS')
    employee_id = fields.Many2one('hr.employee', string='Empleado')
    worked_days_count = fields.Integer(
        string="# Dias Trabajados", default=0.00, compute='_compute_worked_days_count')
    horas_extra_count = fields.Integer(
        string="# Horas Extra", default=0.00, compute='_compute_horas_extra_count')
    ...
    importe_detraccion = fields.Float(
        compute='_compute_importe_detraccion', string='Importe Detraccion')
    rem_1 = fields.Float('Remuneracion 1', readonly=True, default=0.01,
                         compute='_compute_rems')  # Base aportes SIPA
    rem_2 = fields.Float('Remuneracion 2', readonly=True, default=0.01,
                         compute='_compute_rems')  # Base a contribuciones SIPA e INSJP
    # Base contribuciones Fondo Nacional De Empleo/Renatre y Asignaciones
    rem_3 = fields.Float('Remuneracion 3', readonly=True,
                         default=0.01, compute='_compute_rems')
    rem_4 = fields.Float('Remuneracion 4', readonly=True, default=0.01,
                         compute='_compute_rems')  # Base aportes obra social y ANSSAL
    rem_5 = fields.Float('Remuneracion 5', readonly=True, default=0.01,
                         compute='_compute_rems')  # Base aportes INSJP
    rem_6 = fields.Float('Remuneracion 6', readonly=True,
                         default=0.01, compute='_compute_rems')  # No modifica
    rem_7 = fields.Float('Remuneracion 7', readonly=True,
                         default=0.01, compute='_compute_rems')   # No modifica
    rem_8 = fields.Float('Remuneracion 8', readonly=True, default=0.01,
                         compute='_compute_rems')  # Base contribuciones obra social y ANSSAL
    rem_9 = fields.Float('Remuneracion 9', readonly=True, default=0.01,
                         compute='_compute_rems')   # Base LRT (ART)
    # Base contribuciones seguridad social con detraccion
    rem_10 = fields.Float('Remuneracion 10', readonly=True,
                          default=0.01, compute='_compute_rems')
    rem_11 = fields.Float('Remuneracion 11', readonly=True,
                          default=0.01, compute='_compute_rems')  # TODO: Ver que es
    ...
    def _compute_rems(self):
        for record in self:
            # TODO: Deberia agregar un boolean en la structure para setear esto.
            if not (record.employee_id.struct_id.name in 'NO APORT. Y CONTRIB.'):
            ...
            # Caso para empleados fuera de convenio que no liquidan aportes y contribuciones (gerentes)
            else:
                # Solamente liquidan ART.
                record.rem_9 = record.subtotal_bruto + record.subtotal_no_remunerativo
    ...
    def _compute_sicoss_entry_item(self, item):
        ...
        res = {
            ...
            # TODO: Ver varias situaciones de revista
            'sit_revista_1': contract.afip_situacion_revista_id.afip_id,
            'dia_sit_revista_1': 0,  # TODO: Ver varias situaciones de revista
            # TODO: Ver varias situaciones de revista
            'sit_revista_2': contract.afip_situacion_revista_id.afip_id,
            'dia_sit_revista_2': 0,  # TODO: Ver varias situaciones de revista
            # TODO: Ver varias situaciones de revista
            'sit_revista_3': contract.afip_situacion_revista_id.afip_id,
