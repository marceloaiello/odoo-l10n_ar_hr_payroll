...
class PayrollSicossEntry(models.Model):
    ...
    year = fields.Integer(string='Año', required=True, readonly=True,
    ...
    payroll_sicoss_entry_item_ids = fields.One2many(
        'payroll.sicoss_entry.item', 'sicoss_entry_id', string='Liquidacion - por empleado')
    period_ids = fields.One2many(
        'hr.period', 'sicoss_entry_id', string='Liquidacion AFIP-SICOSS')

    @api.depends('year', 'month')
    def _compute_name(self):
        for record in self:
            if record.year and record.month:
                record.name = 'Liquidacion AFIP-SUSS periodo: ' + \
                    str(record.year) + '-' + str(record.month)
    ...
    def button_close(self):
        for record in self:
            if record.state != 'approve':
                raise UserError(
                    'La liquidacion neceista ser aprobada antes de poder cerrarse.')
    ...
    def button_set_to_draft(self):
        for record in self:
            if record.state == 'closed':
                raise UserError(
                    'No puedes volver a borrador una liquidacion que ya fue enviada y contabilizada.')
        ...
        for record in self:
            if self.env['payroll.sicoss_entry'].search_count([('year', '=', record.year), ('month', '=', record.month)]) > 1:
                raise ValidationError(
                    "Ya existe una declaracion jurada existente para el periodo seleccionado.")
